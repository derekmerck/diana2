    ...
    try:
        ...
        def _upload_instance(item):  # Should be all instances
            ...
            if anonymizing:
                ...
                _file = dest.anonymize(item, level=DicomLevel.INSTANCES, replacement_map=map)
                dest.delete(item)
                shammed.file = _file
                dest.put(shammed)
                ...
                dest.gateway.put_metadata(shammed.sham_oid(), DicomLevel.INSTANCES, "Source", source_fn)
